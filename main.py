...
@app.post("/api/productivity/extract")
async def extract_productivity_items(request: dict):
    """Extract todos, calendar events, and notes from conversation"""
    try:
        token = request.get("token")
        ...
        # Extract productivity items
        todos = productivity_service.extract_todos_from_conversation(user_message, ai_response)
        events = productivity_service.extract_calendar_events_from_conversation(user_message, ai_response)
        note = productivity_service.create_note_from_response(user_message, ai_response)
        ...
        # Save todos
        for todo in todos:
            todo_id = await database.save_todo(user_id, chat_id, todo)
            todo["id"] = todo_id
            saved_items["todos"].append(todo)
        
        # Save events
        for event in events:
            event_id = await database.save_calendar_event(user_id, chat_id, event)
            event["id"] = event_id
            saved_items["events"].append(event)
        
        # Save note if response is substantial
        if len(ai_response) > 100:
            note_id = await database.save_note(user_id, chat_id, note)
            note["id"] = note_id
            saved_items["note"] = note
        ...
        return {
            ...
            "summary": {
                ...
                "note_created": len(ai_response) > 100
            }
        }
...
@app.get("/api/productivity/todos/{user_token}")
async def get_user_todos(user_token: str, status: str = "all"):
    """Get user's todos"""
    try:
        user_id = "demo_user"
        if firebase_initialized and user_token:
        ...
        todos = await database.get_user_todos(user_id, status)
        return {"todos": todos}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/productivity/todo/update")
async def update_todo(request: dict):
    ...
    try:
        ...
        success = await database.update_todo(user_id, todo_id, updates)
        return {"success": success}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/productivity/calendar/{user_token}")
async def get_user_calendar_events(user_token: str):
    ...
    try:
        ...
        events = await database.get_user_calendar_events(user_id)
        return {"events": events}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/productivity/event/create")
async def create_calendar_event(request: dict):
    ...
    try:
        ...
        event_id = await database.save_calendar_event(user_id, None, event_data)
        return {"success": True, "event_id": event_id}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/productivity/notes/{user_token}")
async def get_user_notes(user_token: str, category: str = "all"):
    """Get user's notes"""
    try:
        user_id = "demo_user"
        if firebase_initialized and user_token:
        ...
        notes = await database.get_user_notes(user_id, category)
        return {"notes": notes}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/productivity/note/save")
async def save_note_manually(request: dict):
    ...
    try:
        ...
        note_id = await database.save_note(user_id, None, note_data)
        return {"success": True, "note_id": note_id}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/productivity/insights/{user_token}")
async def get_productivity_insights(user_token: str):
    ...
    try:
        ...
        todos = await database.get_user_todos(user_id, "all")
        events = await database.get_user_calendar_events(user_id)
        notes = await database.get_user_notes(user_id, "all")
        
        insights = productivity_service.analyze_productivity_insights(todos, events, notes)
